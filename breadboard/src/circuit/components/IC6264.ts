 ...
 *  Pin  5: A5    Pin 18: D6
 *  Pin  6: A4    Pin 19: D7
 *  Pin  7: A3    Pin 20: !CE  (active LOW chip enable #1)
 *  Pin  8: A2    Pin 21: A10
 *  Pin  9: A1    Pin 22: !OE  (active LOW output enable)
 *  Pin 10: A0    Pin 23: A11
 *  Pin 11: D0    Pin 24: A9
 *  Pin 12: D1    Pin 25: A8
 *  Pin 13: D2    Pin 26: CE2  (active HIGH chip enable #2)
 *  Pin 14: GND   Pin 27: !WE  (active LOW write enable)
 *                Pin 28: VCC
 *
 * Chip enabled: !CE=L AND CE2=H
 * Read:  enabled, !OE=L, !WE=H → drive D0-D7 from memory[addr]
 * Write: enabled, !WE=L        → latch D0-D7 into memory[addr]; D tri-stated
 * Idle:  D0-D7 tri-stated
 */
export class IC6264 extends Component {
  readonly a: Pin[];   // a[0]=A0 .. a[12]=A12 (logical order)
  readonly d: Pin[];   // d[0]=D0 .. d[7]=D7
  readonly ce: Pin;    // !CE (active LOW, pin 20)
  readonly ce2: Pin;   // CE2 (active HIGH, pin 26)
  readonly oe: Pin;    // !OE (active LOW)
  readonly we: Pin;    // !WE (active LOW)
  readonly nc: Pin;    // NC  (pin 1, no connect — pull LOW)
  readonly vcc: Pin;
  readonly gnd: Pin;
  ...
  constructor(label?: string, initialData?: Uint8Array) {
    const nc  = new Pin("NC",  PinRole.INPUT);
    ...
    const ce2 = new Pin("CE2", PinRole.INPUT);
    ...
    super(label ?? "6264", [
      ...
      d0, d1, d2, gnd,
      d3, d4, d5, d6, d7,
      ce, a10, oe, a11, a9, a8, ce2, we, vcc,
    ]);

    this.a = [a0, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11, a12];
    this.d = [d0, d1, d2, d3, d4, d5, d6, d7];
    this.ce = ce; this.ce2 = ce2; this.oe = oe; this.we = we;
    this.nc = nc; this.vcc = vcc; this.gnd = gnd;

    this.data = new Uint8Array(8192); // 8KB, initialized to 0
    if (initialData) this.data.set(initialData.subarray(0, 8192));
  }

  evaluate(): void {
    const chipEnabled  = !this.ce.logicLevel && this.ce2.logicLevel;
